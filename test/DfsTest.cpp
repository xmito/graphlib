...
TEST_CASE("Traversal tests on ListDiGraph") {
    ...
    using node_handle =
        typename ListDiGraph<TraversableNodeData, EdgeData>::node_handle;
    ...
    SECTION("Test with random set of edges") {
        ...
        syslog(LOG_INFO | LOG_USER,
               "DFSTEST -> Traversal test on ListDiGraph, seed: %u", seed);
...
TEST_CASE("Traversal tests on ListGraph") {
    using namespace graphlib;
    using node_handle = typename graph_traits<
        ListGraph<TraversableNodeData, EdgeData>>::node_handle;
