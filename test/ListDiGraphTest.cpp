...
TEST_CASE("ListDiGraph::addEdge") {
    ...
    SECTION("ListDiGraph::addEdge() without weight") {
        ListDiGraph<TraversableNodeData, EdgeData> graph;
        ListDiGraph<TraversableNodeData, EdgeData>::node_handle
            node_handles[20];
        ListDiGraph<TraversableNodeData, EdgeData>::edge_handle
            edge_handles[19];
        for (int i = 0; i < 20; ++i) {
            ...
            if (i > 0) {
                ...
                REQUIRE(graph.getSource(edge_handles[i - 1]) ==
                        node_handles[i - 1]);
                REQUIRE(graph.getTarget(edge_handles[i - 1]) ==
                        node_handles[i]);
            }
        }
    }
    SECTION("ListDiGraph::addEdge() with weight") {
        ...
        ListDiGraph<TraversableNodeData, WeightedEdgeData>::node_handle
            node_handles[20];
        ListDiGraph<TraversableNodeData, WeightedEdgeData>::edge_handle
            edge_handles[19];
        for (int i = 0; i < 20; ++i) {
            node_handles[i] = graph.addNode();
            if (i > 0) {
...
TEST_CASE("ListDiGraph lookup methods") {
    using namespace graphlib;
    using Graph =
        ListDiGraph<LocationNodeData<PlaneLocation>, WeightedEdgeData>;
    ...
    SECTION("ListDiGraph::getNode(const node_handle &)") {
        ...
        auto &cndata =
            const_cast<const Graph &>(graph).getNode(node_handles[3]);
    ...
    SECTION("ListDiGraph::getSourceNode/getTargetNode(const edge_handle &)") {
        ...
        auto &csrc_node =
            const_cast<const Graph &>(graph).getSourceNode(edge_handles[5]);

        auto &tg_node = graph.getTargetNode(edge_handles[5]);
        REQUIRE(tg_node.color_ == Color::WHITE);
        REQUIRE(tg_node.pred_ == NodeHandle());
        auto &ctg_node =
            const_cast<const Graph &>(graph).getTargetNode(edge_handles[5]);
    }
...
TEST_CASE("ListDiGraph modifiers") {
    ...
    SECTION(
        "ListDiGraph::setNodePred(const node_handle &, const node_handle&)") {
        graph.setNodePred(node_handles[0], node_handles[1]);
        REQUIRE(graph.getNodePred(node_handles[0]) == node_handles[1]);
    }
    SECTION(
        "ListDiGraph::setNodeLoc(const node_handle &, const location_type &)") {
