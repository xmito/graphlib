...
struct FibonacciHeap
{
  ...
  public:
    struct Node
    {
        ...
        explicit Node(const value_type &key) : mark_(false), key_(key), parent_(nullptr) {}
        Node(const value_type &key, const_iterator parent) : mark_(false), key_(key), parent_(parent) {}
        Node(const Node &node) : mark_(node.mark_),
                                 key_(node.key_), parent_(node.parent_), clist_(node.clist_) {}
        Node(Node &&node) noexcept : mark_(node.mark_), key_(std::move(node.key_)),
    ...
    FibonacciHeap(InputIt first, InputIt last, const Compare &comp) : nodes_(0), top_(nullptr), comp_(comp)
    {
        while (first != last)
            push(*first++);
    }
    FibonacciHeap(std::initializer_list<value_type> ilist, const Compare &comp) : FibonacciHeap(ilist.begin(), ilist.end(), comp) {}
    explicit FibonacciHeap(const Compare &comp) : nodes_(0), comp_(comp) {}
    explicit FibonacciHeap(const Graph &graph) : FibonacciHeap(graph.beginNode(), graph.endNode(), Compare(&graph)) {}
    FibonacciHeap(const Graph &graph, const Compare &comp) : FibonacciHeap(graph.beginNode(), graph.endNode(), comp) {}
    FibonacciHeap(const FibonacciHeap &fh) : nodes_(fh.nodes_), rlist_(fh.rlist_), comp_(fh.comp_), map_(fh.map_)
    {
    ...
    FibonacciHeap(FibonacciHeap &&fh) noexcept : nodes_(fh.nodes_), top_(std::move(fh.top_)),
