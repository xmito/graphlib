...
TEST_CASE("FibonacciHeap::FibonacciHeap(*)/operator=") {
    ...
    SECTION(
        "FibonacciHeap(std::initializer_list<value_type>, const Compare&)") {
        FibonacciHeap<Graph> fh(
            {handles[0], handles[1], handles[2], handles[3]}, comp);
        REQUIRE(fh.size() == 4);
...
TEST_CASE("FibonacciHeap modifiers") {
    ...
    for (int i = 0; i < 20; ++i) {
        if (i > 0)
            handles.push_back(
                graph.addNode(Color::BLACK, handles[i - 1], i + 20));
        else
